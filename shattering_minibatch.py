...
def train(optimizer, X, y, n_batches, batch_size, lr_init):
    print("N_Batches: {} of {} samples each".format(n_batches, batch_size))
    for i in xrange(0, n_batches):
        #lr = float(lr_init)*((0.99999)**(i*batch_size))
        
        u = np.random.choice(np.arange(X.shape[0]))
                
        loss = optimizer.train(X[u:u+batch_size], y[u:u+batch_size], lr_init)
        ...
        if error == 0:
            break
        if i % 1000 == 0:
            print("Batch {}, error:{}, loss:{}".format(i, error, loss))
...
if __name__ == "__main__":
    ...
    num_pts = range(1000, 100000, 100)# + range(500, 1100, 50)
    ...
    for repeat in range(num_repeats):
        for H_idx, H in enumerate(Hs):
            for N_idx, N in enumerate(num_pts):
                ...
                n_batches = int(np.ceil(float(N) / float(batch_size) * iters_per_pt))
                ...
                while (nan_loss and repeat_count < num_repeats):
                    ...
                    sgd_optimizer = SGD(mlp, LogLoss(), clip_gradients=0.1)
                    
                    # Train the network until we reach 0 error or n_iter iterations
                    error, iterations = train(sgd_optimizer, X, labels, n_batches, batch_size, lr)
                ...
                print("Finished {} Pts, Error: {}".format(N, error))
